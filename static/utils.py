#!/usr/bin/env python
import html, io, json, os, pickle, random, re, sqlite3, string, sys, subprocess, time, traceback, urllib.parse, urllib.request, xml.etree.ElementTree
import lxml.etree
...
def query_obj(obj):
    # ?kind=obj&v=cos%0Asystem%0A(S%27ping%20-c%205%20127.0.0.1%27%0AtR.%0A
    content = str(pickle.loads(obj.encode()))
    return content


def query_id(id_):
    # ?kind=id&v=12%20or%201=1
    cursor = connection.cursor()
    cursor.execute("SELECT id, username, name, surname FROM users WHERE id=" + id_)
...
def query_version(version):
    # ?kind=version&v=xxx<img%20src=""%20onerror="alert(1)">
    content = re.sub(r"(v<b>)[^<]+(</b>)", r"\g<1>%s\g<2>" % version, HTML_POSTFIX)
    return content


def query_path(path):
    # ?kind=path&v=/etc/passwd
    content = (open(os.path.abspath(path), "rb") if not "://" in path else urllib.request.urlopen(path)).read().decode()
    return content


def query_domain(domain):
    # ?kind=domain&v=baidu.com;whoami
    content = subprocess.check_output("nslookup " + domain, shell=True, stderr=subprocess.STDOUT, stdin=subprocess.PIPE).decode()
    return content


def query_xml(value):
    # TODO
    # ?kind=xml&v=<!DOCTYPE%20example%20%5B<!ENTITY%20xxe%20SYSTEM%20"http%3A%2F%2Fpastebin.com%2Fraw.php%3Fi%3Dh1rvVnvx">%5D><root>%26xxe%3B<%2Froot>
    content = lxml.etree.tostring(lxml.etree.parse(io.BytesIO(value.encode()), lxml.etree.XMLParser(no_network=False)), pretty_print=True).decode()
    return content


def query_name(name):
    # TODO
    found = lxml.etree.parse(io.BytesIO(USERS_XML.encode())).xpath(".//user[name/text()='%s']" % name)
...
def query_include(v):
    ...
    program = (open(v, "rb") if not "://" in v else urllib.request.urlopen(v)).read()
    envs = {"DOCUMENT_ROOT": os.getcwd(), "HTTP_USER_AGENT": self.headers.get("User-Agent"), "REMOTE_ADDR": self.client_address[0], "REMOTE_PORT": self.client_address[1], "PATH": path, "QUERY_STRING": query}
    exec(program, envs)
...
def query_url(v):
    content = HTML_PREFIX
    content = content.replace("<head>", "<head><meta http-equiv=\"refresh\" content=\"0; url=%s\"/>" % v)
    return content
